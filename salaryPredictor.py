...
def obtain_stats_for_prediction(connection, player, weights = [0.2, 1, 0.85, 0.7, 0.55, 0.4]):
    """
...
def obtain_training_data(connection, weights = [0.2, 1, 0.85, 0.7, 0.55, 0.4]):
    """
...
def train_xgboost(training_data, test_size=0.25, random_state=42):
    """
    Train an XGBoost model with provided training data
    
    Arguments:
        training_data = DataFrame containing the training data for the mode
        test_size = Fraction of dataset reserved for testing
        random_state = Seed initialization for train_test_split and XGBoost
        
    Return: 
        Trained XGBoost model
    """
    
    # Separate the features from the salart we are predicting
    features = training_data.drop(columns=["salary_cap_fraction"])
    salary = training_data["salary_cap_fraction"]
    
    # Split the data into training and testing data
    features_train, features_test, salary_train, salary_test = train_test_split(features, salary, test_size=test_size, random_state=random_state)
    
    # Initialize the model
    xgboost_model = xgb.XGBRegressor(objective='reg:squarederror', n_estimators=100, random_state=random_state, reg_alpha=0.1, reg_lambda=10, max_depth=4, min_child_weight=2, subsample=0.7)
    
    # Train the model
    xgboost_model.fit(features_train, salary_train)
    
    # Print a quality estimator in case there is a testing set
    if test_size > 0:
        print("XGBoost: R^2 score on testing data: {}".format(r2_score(salary_test, xgboost_model.predict(features_test))))

    # Return the trained model
    return xgboost_model


def predict_salaries(connection, model, player_data):
    """
    ...
    player_data["salary"] = predictions
    ...
    for player in player_data.itertuples():
        print("XGBoost salary prediction for {} {} for season 2026 is {:.0f}".format(player.first_name, player.last_name, round(player.salary * salary_cap, -3)))
...
def main():
    ...
    xgboost_model = train_xgboost(training_data)
    ...
    predict_salaries(connection, xgboost_model, prediction_data)
